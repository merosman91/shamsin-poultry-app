  ...
  useEffect(() => {
    if (id) {
      const fetchBatchData = async () => {
        ...
        if (!batchData) {
          router.push('/batches'); // العودة إذا لم توجد الدفعة
          return;
        }
        setBatch(batchData);
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="p-4 bg-white rounded shadow">
          <h3 className="font-semibold">السلالة</h3>
          <p>{batch.breed}</p>
        </div>
        <div className="p-4 bg-white rounded shadow">
          <h3 className="font-semibold">عدد الكتاكيت</h3>
          <p>{batch.chickCount}</p>
        </div>
        <div className="p-4 bg-white rounded shadow">
          <h3 className="font-semibold">تاريخ البدء</h3>
          <p>{new Date(batch.startDate).toLocaleDateString('ar-EG')}</p>
        </div>
      </div>

      {/* نموذج التسجيل اليومي */}
      <div className="bg-white p-6 rounded-lg shadow mb-6">
        <h2 className="text-2xl font-bold mb-4">التسجيل اليومي</h2>
        <DailyLogForm batchId={batch.id} onLogAdded={() => {/* يمكن تحديث البيانات هنا */}} />
      </div>
